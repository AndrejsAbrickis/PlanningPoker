...
namespace PlanningPoker.Core {
    ...
    public class InMemoryUserTracker<THub> : IUserTracker<THub> {
        private readonly ConcurrentDictionary<string, UserDetailsDto> _usersOnline = new ConcurrentDictionary<string, UserDetailsDto> ();
        ...
        public Task<UserDetailsDto> GetUser (string connectionId) {
            ...
            if (user == null) {
                user = new UserDetailsDto (connectionId, string.Empty, string.Empty);
            }
